...
export default function Watchlist({ symbols, setSymbols, prices }) {
  ...
  const addSymbol = () => { const s = newSym.trim().toUpperCase(); if (!s) return; if (!/^[A-Z0-9]{3,15}$/.test(s)) return; const withUsdt = s.endsWith('USDT') ? s : `${s}USDT`; if (symbols.includes(withUsdt)) return; setSymbols([withUsdt, ...symbols]); setNewSym(''); };
  const removeSymbol = (sym) => setSymbols(symbols.filter((x) => x !== sym));
  return (
    <div className="rounded-xl border border-white/10 bg-zinc-900/60 p-4">
      <div className="mb-3 flex items-center justify-between"><div className="text-sm font-semibold text-white/90">Watchlist</div></div>
      ...
      <div className="divide-y divide-white/5">
        ...
        {rows.length === 0 && (<div className="py-6 text-center text-xs text-white/50">No symbols tracked. Add some above.</div>)}
      </div>
    </div>
  );
}
